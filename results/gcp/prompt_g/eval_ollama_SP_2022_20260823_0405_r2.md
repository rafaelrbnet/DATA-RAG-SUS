...
SELECT 
  cid_principal,
  COUNT(CASE WHEN obito_hospital = 'SIM' THEN 1 END) AS mortes,
  COUNT(DISTINCT n_aih) AS internacoes,
  ROUND(100.0 * COUNT(CASE WHEN obito_hospital = 'SIM' THEN 1 END) / COUNT(DISTINCT n_aih), 2) AS taxa_mortalidade
FROM 
  processed
WHERE 
  sistema = 'SIH'
...
**Erro:** Falha DuckDB: Binder Error: Referenced column "obito_hospital" not found in FROM clause!
Candidate bindings: "custo_total", "cid_principal", "cgc_hosp", "gestao_responsavel", "pa_motsai"

LINE 3:   COUNT(CASE WHEN obito_hospital = 'SIM' THEN 1 END) AS mortes,
                          ^
...
**Resultado:** `Linha 1: got={'internacoes_idosos': 19120.0, 'internacoes_total': 29359} exp={'total_s72': 29356, 's72_idosos': 19120, 'pct_idosos': 65.1}`
**TTA:** 5084ms
...
WHERE sistema = 'SIH'
  ...
  AND ano_cmpt = 2022
  AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98')
  AND cod_munic_paciente <> cod_munic_estabelecimento;
```
...
LINE 8:   AND cod_munic_paciente <> cod_munic_estabelecimento;
              ^
...
**SQL gerado:**
```sql
SELECT ROUND(AVG(custo_total / dias_perm), 2) AS custo_medio_por_dia
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
GROUP BY 
  cnes_estabelecimento
ORDER BY 
  media_internacoes DESC;
```

**Erro:** Falha DuckDB: Binder Error: aggregate function calls cannot be nested

LINE 3:   ROUND(AVG(COUNT(DISTINCT n_aih)), 1) AS media_internacoes
                    ^
...
**SQL gerado:**
```sql
SELECT ROUND(AVG(custo_total), 2) AS custo_medio_internacao
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**SQL gerado:**
```sql
SELECT ROUND(AVG(custo_total), 2) AS custo_medio_procedimento
FROM processed
WHERE sistema = 'SIA'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
SELECT 
  mes_cmpt,
  ROUND(SUM(CASE WHEN obito = 'S' THEN 1 ELSE 0 END) * 100.0 / COUNT(DISTINCT n_aih), 2) AS taxa_mortalidade
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
LINE 3:   ROUND(SUM(CASE WHEN obito = 'S' THEN 1 ELSE 0 END) * 100.0 / COUNT(DISTINCT...
                              ^
...
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_...`
- **Q19** (Epidemiológica Complexa): Falha DuckDB: Binder Error: Referenced column "obito_hospital" not found in FROM clause!
Candidate bindings: "custo_total", "cid_principal", "cgc_hosp", "gestao_responsavel", "pa_motsai"
...
- **Q21** (Epidemiológica Complexa): Linha não encontrada: {'sexo_paciente': 'M', 'total': 40123, 'pct': 60.8}
  ...
  ROUND(100.0 * COUNT(*) / SUM(COUNT(*)) OVER (), 1) AS pct
FR...`
- **Q22** (Epidemiológica Complexa): Linha 1: got={'internacoes_idosos': 19120.0, 'internacoes_total': 29359} exp={'total_s72': 29356, 's72_idosos': 19120, 'pct_idosos': 65.1}
  SQL: `SELECT
  SUM(CASE WHEN idade_paciente >= 60 THEN 1 ELSE 0 END) AS internacoes_idosos,
  COUNT(*) AS internacoes_total
FR...`
...
WHERE sistema = 'SIH'
  AND uf_origem = 'SP...`
- **Q37** (Financeira): got=None exp=432.54
  SQL: `SELECT ROUND(AVG(custo_total / dias_perm), 2) AS custo_medio_por_dia
FROM processed
WHERE sistema = 'SIH'
  AND uf_orige...`
...
LINE 3:   ROUND(SUM(CASE WHEN obito = 'S' THEN 1 ELSE 0 END) * 100.0 / COUNT(DISTINCT...
  ...
  SQL: `SELECT 
  mes_cmpt,
  ROUND(SUM(CASE WHEN obito = 'S' THEN 1 ELSE 0 END) * 100.0 / COUNT(DISTINCT n_aih), 2) AS taxa_mor...`
