...
| Correct via flexible match (alias diferente) | 83 |
| API errors / timeouts | 0 |
| Mean Time-to-Answer | 2.3s |
...
SELECT sexo_paciente,
  ...
  ROUND(100.0 * COUNT(DISTINCT CASE WHEN morte = 1 THEN n_aih END) / COUNT(DISTINCT n_aih), 1) AS taxa_mortalidade_pct
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'mes_cmpt': 1, 'obitos': 179, 'total_internacoes': 5209, 'taxa_mortalidade_pct': 3.44} exp={'mes_cmpt': 1, 'internacoes': 5209, 'obitos': 179.0, 'taxa_mortalidade': 3.44}`
**TTA:** 2338ms
...
WHERE sistema = 'SIA'
  AND uf_origem = 'SP'
  AND ...`
- **Q109** (Temporal/Comparativa): Linha 1: got={'mes_cmpt': 1, 'obitos': 179, 'total_internacoes': 5209, 'taxa_mortalidade_pct': 3.44} exp={'mes_cmpt': 1, 'internacoes': 5209, 'obitos': 179.0, 'taxa_mortalidade': 3.44}
  SQL: `SELECT
  mes_cmpt,
  COUNT(DISTINCT CASE WHEN morte = 1 THEN n_aih END) AS obitos,
  COUNT(DISTINCT n_aih) AS total_inte...`
