...
| Correct via flexible match (alias diferente) | 71 |
| API errors / timeouts | 0 |
| Mean Time-to-Answer | 10.1s |
...
**Resultado:** `Linha 1: got={'cid_principal': 'T056', 'obitos': 1, 'internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 14570ms
...
```sql
SELECT
  ROUND(100.0 * COUNT(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN n_aih END) / COUNT(n_aih), 1) AS pct_internacoes_outro_municipio
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
```sql
SELECT 
  ROUND(SUM(COALESCE(custo_total, 0)) / SUM(COALESCE(qt_diarias, 0)), 2) AS custo_medio_por_dia
FROM 
  processed
WHERE 
  sistema = 'SIH'
...
```sql
SELECT cid_principal,
  COUNT(DISTINCT n_aih) AS internacoes_obitidas
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
  ...
  AND morte = 1
GROUP BY cid_principal
ORDER BY internacoes_obitidas DESC
LIMIT 5;
```

**Resultado:** `Linha 1: got={'cid_principal': 'S720', 'internacoes_obitidas': 455} exp={'cid_principal': 'S720', 'total_internacoes': 8980, 'total_obitos': 455.0}`
**TTA:** 10262ms
...
SELECT 
  sexo_paciente,
  COUNT(CASE WHEN morte = 1 THEN n_aih END) AS obitos,
  COUNT(DISTINCT n_aih) AS total_internacoes,
...
- **Q18** (Epidemiológica Complexa): Linha não encontrada: {'faixa_etaria': '0-17', 'total': 3619}
  ...
  CASE
    WHEN idade_paciente BETWEEN 0 AND 17 THEN '0-17'
    WHEN idade_paciente BETWEEN 18 AND 59 THEN '18-59...`
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T056', 'obitos': 1, 'internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(DISTINCT CASE WHEN morte = 1 THEN n_aih END) AS obitos,
...
- **Q28** (Epidemiológica Complexa): Linha 1: got={'pct_internacoes_outro_municipio': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  SQL: `SELECT
  ROUND(100.0 * COUNT(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN n_aih END) / COUNT(n_aih),...`
...
- **Q70** (Epidemiológica Complexa): Número de linhas: got=8 exp=3
  ...
  CASE 
    ...
    WHEN idade_paciente < 30 THEN '18-29'
    WHEN idade_pacien...`
- **Q71** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'S720', 'internacoes_obitidas': 455} exp={'cid_principal': 'S720', 'total_internacoes': 8980, 'total_obitos': 455.0}
  SQL: `SELECT cid_principal,
  COUNT(DISTINCT n_aih) AS internacoes_obitidas
FROM processed
WHERE sistema = 'SIH'
