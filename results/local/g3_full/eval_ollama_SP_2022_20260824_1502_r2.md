...
**Resultado:** `Linha 1: got={'cid_principal': 'T056', 'mortes': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 14613ms
...
SELECT
  ...
  (SELECT COUNT(DISTINCT n_aih)
   ...
   WHERE sistema = 'SIH'
     ...
     AND ano_cmpt = 2022
     AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98')
     AND mes_cmpt = p.mes_cmpt) AS total_sih
FROM processed p
WHERE sistema = 'SIA'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `got=488.68 exp=432.54`
**TTA:** 9035ms
...
**Resultado:** `Linha 1: got={'cod_munic_residencia': '355030', 'procedimentos': 31889} exp={'cod_munic_estabelecimento': '355030', 'total': 33846}`
**TTA:** 9304ms
...
**Resultado:** `Linha 1: got={'cid_principal': 'S063', 'permanencia_media_dias': 16.4, 'internacoes': 78} exp={'cid_principal': 'S063', 'internacoes': 78, 'permanencia_media': 16.4}`
**TTA:** 11696ms
...
SELECT
  mes_cmpt,
  ROUND(100.0 * SUM(CASE WHEN morte = 1 THEN 1 ELSE 0 END) / COUNT(DISTINCT n_aih), 1) AS taxa_mortalidade_pct
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'mes_cmpt': 1, 'taxa_mortalidade_pct': 3.4} exp={'mes_cmpt': 1, 'internacoes': 5209, 'obitos': 179.0, 'taxa_mortalidade': 3.44}`
**TTA:** 11413ms
...
- **Q18** (Epidemiológica Complexa): Linha não encontrada: {'faixa_etaria': '0-17', 'total': 3619}
  ...
  CASE
    WHEN idade_paciente BETWEEN 0 AND 17 THEN '0-17'
    WHEN idade_paciente BETWEEN 18 AND 59 THEN '18-59...`
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T056', 'mortes': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(CASE WHEN morte = 1 THEN n_aih END) AS mortes,
...
WHERE sistema = 'SIH'
  AND uf_origem = 'SP...`
- **Q37** (Financeira): got=488.68 exp=432.54
...
WHERE sistema = 'SIA'
  AND uf_origem = '...`
- **Q69** (Epidemiológica Complexa): Linha 1: got={'cod_munic_residencia': '355030', 'procedimentos': 31889} exp={'cod_munic_estabelecimento': '355030', 'total': 33846}
...
- **Q75** (Epidemiológica Complexa): Número de linhas: got=16 exp=6
  ...
  CASE 
    ...
    WHEN idade_paciente < 30 THEN '18-29'
    WHEN...`
- **Q76** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'S063', 'permanencia_media_dias': 16.4, 'internacoes': 78} exp={'cid_principal': 'S063', 'internacoes': 78, 'permanencia_media': 16.4}
  SQL: `SELECT 
  cid_principal,
  ROUND(AVG(dias_perm), 1) AS permanencia_media_dias,
  COUNT(DISTINCT n_aih) AS internacoes
...
- **Q83** (Epidemiológica Complexa): got=2221.0 exp=173.1
  SQL: `SELECT cnes_estabelecimento,
  ROUND(COUNT(DISTINCT n_aih) / COUNT(DISTINCT cod_munic_estabelecimento), 2) AS media_inte...`
...
- **Q99** (Financeira): Linha 1: got={'cod_procedimento': '0702100048', 'total_procedimentos': 15807, 'custo_total': 46893518.21} exp={'cod_procedimento': '0701020369', 'custo_total': 1201460.4}
  ...
  ROUND(SUM(COALESCE(custo_total, 0)), 2) AS custo_total
FRO...`
- **Q109** (Temporal/Comparativa): Linha 1: got={'mes_cmpt': 1, 'taxa_mortalidade_pct': 3.4} exp={'mes_cmpt': 1, 'internacoes': 5209, 'obitos': 179.0, 'taxa_mortalidade': 3.44}
  SQL: `SELECT
  mes_cmpt,
