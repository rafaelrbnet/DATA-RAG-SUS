...
|---|---|
| Total queries | 118 |
| Correct (EA numerator) | 91 |
| **Execution Accuracy (EA)** | **77.1%** |
| Wilson IC 95% | [68.8%, 83.8%] |
...
```sql
SELECT 
  ROUND(100.0 * COUNT(CASE WHEN clinical_deslocamento_territorial = 'deslocamento intermunicipal' THEN n_aih END) / COUNT(DISTINCT n_aih), 1) AS pct_intermunicipal
FROM processed
...
**Resultado:** `Linha 1: got={'pct_intermunicipal': 56.0} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}`
**TTA:** 12571ms
...
WHERE sistema = 'SIH'
  AND uf_origem = '...`
- **Q28** (Epidemiológica Complexa): Linha 1: got={'pct_intermunicipal': 56.0} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  SQL: `SELECT 
  ROUND(100.0 * COUNT(CASE WHEN clinical_deslocamento_territorial = 'deslocamento intermunicipal' THEN n_aih END...`
