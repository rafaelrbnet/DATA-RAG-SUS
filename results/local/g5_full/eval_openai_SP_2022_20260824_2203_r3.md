...
**Erro:** Falha SQL: Connection error.
...
SELECT
  CASE
    WHEN mes_cmpt BETWEEN 1 AND 6 THEN 'S1'
    ELSE 'S2'
  END AS semestre,
  ROUND(SUM(COALESCE(custo_total, 0)), 2) AS custo_total
FROM processed
...
WHERE sistema = 'SIH'
  AND uf_origem = 'SP...`
- **Q75** (Epidemiológica Complexa): Falha SQL: Connection error.
...
- **Q84** (Epidemiológica Complexa): Número de linhas: got=5 exp=4
  ...
  CASE
    ...
    WHEN dias_perm BETWEEN 4 AND 7 THEN '4-7 dias'
   ...`
- **Q90** (Financeira): Falha SQL: Connection error.
