...
| API errors / timeouts | 6 |
| Mean Time-to-Answer | 32.5s |
...
**Resultado:** `Linha 1: got={'cid_principal': 'T181', 'obitos': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 45120ms
...
WITH total_internacoes AS (
  SELECT COUNT(DISTINCT n_aih) AS total
  FROM processed
  WHERE sistema = 'SIH'
    AND uf_origem = 'SP'
    AND ano_cmpt = 2022
...
internacoes_femur AS (
  SELECT COUNT(DISTINCT n_aih) AS total_femur
  FROM processed
  WHERE sistema = 'SIH'
    AND uf_origem = 'SP'
    AND ano_cmpt = 2022
...
SELECT 
  (SELECT total_femur FROM internacoes_femur) AS internacoes_femur,
  (SELECT total FROM total_internacoes) AS total_internacoes,
  ROUND(100.0 * (SELECT total_femur FROM internacoes_femur) / (SELECT total FROM total_internacoes), 1) AS pct_femur
;
```
...
SELECT
  ROUND(
    100.0 * COUNT(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN n_aih END) /
...
**Resultado:** `Linha 1: got={'proporcao': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}`
**TTA:** 35723ms
...
WHERE sistema = 'SIH'
  ...
  AND ano_cmpt = 2022
  AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98')
  AND uti_passagem = 'S';  -- Assuming there's a column 'uti_passagem' indicating if the patient passed through the ICU
```
...
LINE 7:   AND uti_passagem = 'S';  -- Assuming there's a column 'uti_pass...
              ^
...
- **Q18** (Epidemiológica Complexa): Linha não encontrada: {'faixa_etaria': '0-17', 'total': 3619}
  ...
  CASE
    WHEN idade_paciente BETWEEN 0 AND 17 THEN '0-17'
    WHEN idade_paciente BETWEEN 18 AND 59 THEN '18-59...`
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T181', 'obitos': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(DISTINCT CASE WHEN morte = 1 THEN n_aih END) AS obitos,
...
WHERE sistema = 'SIH'
  AND uf_origem = '...`
- **Q28** (Epidemiológica Complexa): Linha 1: got={'proporcao': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  SQL: `SELECT
  ROUND(
    100.0 * COUNT(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN n_aih END) /
    COUN...`
...
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_...`
- **Q67** (Epidemiológica Simples): got=28948 exp=1216
