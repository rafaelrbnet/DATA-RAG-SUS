...
**Resultado:** `Linha 1: got={'cid_principal': 'T181', 'total_internacoes': 1, 'obitos': 1.0, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 30132ms
...
**Resultado:** `Linha 1: got={'proporcao_sh_vs_sp': 472.05} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}`
**TTA:** 9696ms
...
SELECT
  ...
  ROUND(100.0 * SUM(CASE WHEN morte = 1 THEN 1 ELSE 0 END) / COUNT(DISTINCT n_aih), 1) AS taxa_obito
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
- **Q18** (Epidemiológica Complexa): Linha não encontrada: {'faixa_etaria': '0-17', 'total': 3619}
  ...
  CASE
    WHEN idade_paciente BETWEEN 0 AND 17 THEN '0-17'
    WHEN idade_paciente BETWEEN 18 AND 59 THEN '18-59...`
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T181', 'total_internacoes': 1, 'obitos': 1.0, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT
  cid_principal,
  COUNT(DISTINCT n_aih) AS total_internacoes,
  SUM(CASE WHEN morte = 1 THEN 1 ELSE 0 END) AS ob...`
...
FROM processed
WHERE sistema = 'SI...`
- **Q40** (Financeira): Linha 1: got={'proporcao_sh_vs_sp': 472.05} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}
  SQL: `SELECT
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / SUM(COALESCE(val_sp, 0)), 2) AS proporcao_sh_vs_sp
FROM processed
WHER...`
