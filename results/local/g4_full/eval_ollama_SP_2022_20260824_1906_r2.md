...
|---|---|
| Total queries | 118 |
| Correct (EA numerator) | 81 |
| **Execution Accuracy (EA)** | **68.6%** |
| Wilson IC 95% | [59.8%, 76.3%] |
| Correct via flexible match (alias diferente) | 66 |
| API errors / timeouts | 9 |
| Mean Time-to-Answer | 11.0s |
...
SELECT cid_principal,
  COUNT(DISTINCT n_aih) AS total_internacoes,
  COUNT(CASE WHEN ocorreu_obito = 'SIM' THEN 1 END) AS obitos,
  ROUND(100.0 * COUNT(CASE WHEN ocorreu_obito = 'SIM' THEN 1 END) / COUNT(DISTINCT n_aih), 1) AS taxa_mortalidade
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
LINE 3:   COUNT(CASE WHEN ocorreu_obito = 'SIM' THEN 1 END) AS obitos,
                          ^
...
```sql
SELECT 
  ROUND(100.0 * COUNT(DISTINCT CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN n_aih END) / COUNT(DISTINCT n_aih), 1) AS pct_internacoes_outro_municipio
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
SELECT 
  sexo_paciente,
  COUNT(DISTINCT n_aih) AS total_internacoes,
  SUM(CASE WHEN outcome = 'ÓBITO' THEN 1 ELSE 0 END) AS obitos,
  ROUND(100.0 * SUM(CASE WHEN outcome = 'ÓBITO' THEN 1 ELSE 0 END) / COUNT(DISTINCT n_aih), 1) AS taxa_mortalidade
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
LINE 4:   SUM(CASE WHEN outcome = 'ÓBITO' THEN 1 ELSE 0 END) AS obitos,
                        ^
...
SELECT
  CASE
    WHEN dias_perm < 5 THEN '0-4'
    WHEN dias_perm BETWEEN 5 AND 10 THEN '5-10'
    WHEN dias_perm BETWEEN 11 AND 15 THEN '11-15'
    WHEN dias_perm BETWEEN 16 AND 20 THEN '16-20'
    ELSE '21+'
...
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt ...`
- **Q14** (Epidemiológica Simples): got=64870 exp=3
  SQL: `SELECT COUNT(DISTINCT n_aih) AS total_internacoes
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
...
LINE 3:   COUNT(CASE WHEN ocorreu_obito = 'SIM' THEN 1 END) AS obitos,
  ...
  COUNT(DISTINCT n_aih) AS total_internacoes,
  COUNT(CASE WHEN ocorreu_obito = 'SIM' THEN 1 END) ...`
...
- **Q28** (Epidemiológica Complexa): Linha 1: got={'pct_internacoes_outro_municipio': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  SQL: `SELECT 
  ROUND(100.0 * COUNT(DISTINCT CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN n_aih END) / COU...`
...
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt =...`
- **Q61** (Epidemiológica Simples): got=4 exp=89
  SQL: `SELECT COUNT(DISTINCT n_aih) AS total_internacoes
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
...
LINE 4:   SUM(CASE WHEN outcome = 'ÓBITO' THEN 1 ELSE 0 END) AS obitos,
  ...
  sexo_paciente,
  COUNT(DISTINCT n_aih) AS total_internacoes,
  SUM(CASE WHEN outcome = 'ÓBITO' THEN 1 ELSE 0 E...`
...
- **Q88** (Financeira): Linha não encontrada: {'sexo_paciente': 'F', 'custo_medio': 2684.14}
  ...
  COUNT(DISTINCT n_aih) AS total_internacoes,
  ROUND(SUM(COALESCE(custo_total, 0)) / COUNT(DISTIN...`
