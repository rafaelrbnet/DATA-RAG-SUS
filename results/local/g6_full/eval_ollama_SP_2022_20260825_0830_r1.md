...
```sql
SELECT
  ROUND(100.0 * COUNT(CASE WHEN clinical_deslocamento_territorial = 'deslocamento intermunicipal' THEN n_aih END) / COUNT(n_aih), 1) AS pct_internacoes_intermunicipais
FROM processed
...
**Resultado:** `Linha 1: got={'pct_internacoes_intermunicipais': 56.0} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}`
**TTA:** 12728ms
...
WHERE sistema = 'SIH'
  AND uf_origem = '...`
- **Q28** (Epidemiológica Complexa): Linha 1: got={'pct_internacoes_intermunicipais': 56.0} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  SQL: `SELECT
