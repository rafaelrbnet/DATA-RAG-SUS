...
**Resultado:** `Linha 1: got={'cid_principal': 'S279', 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 7692ms
...
```sql
SELECT
  ROUND(100.0 * SUM(CASE WHEN e.clinical_deslocamento_territorial = 'deslocamento intermunicipal' THEN 1 ELSE 0 END) / COUNT(DISTINCT n_aih), 1) AS pct_deslocamento_intermunicipal
FROM processed
...
**Resultado:** `Linha 1: got={'pct_servico_hospitalar': 82.52, 'pct_honorarios': 17.48} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}`
**TTA:** 8241ms
...
**Resultado:** `Linha 1: got={'cid_principal': 'S063', 'permanencia_media_dias': 16.4} exp={'cid_principal': 'S063', 'internacoes': 78, 'permanencia_media': 16.4}`
**TTA:** 8171ms
...
- **Q18** (Epidemiológica Complexa): Linha não encontrada: {'faixa_etaria': '0-17', 'total': 3619}
  ...
  CASE
    ...
    WHEN idade_paciente BETWEEN 18 AND 59 THEN '18-59'
    ELSE ...`
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'S279', 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
...
FROM processed
WHERE sistema = 'SI...`
- **Q40** (Financeira): Linha 1: got={'pct_servico_hospitalar': 82.52, 'pct_honorarios': 17.48} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}
  SQL: `SELECT
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / SUM(COALESCE(custo_total, 0)), 2) AS pct_servico_hospitalar,
  ROUND(1...`
...
- **Q76** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'S063', 'permanencia_media_dias': 16.4} exp={'cid_principal': 'S063', 'internacoes': 78, 'permanencia_media': 16.4}
  SQL: `SELECT cid_principal,
  ROUND(AVG(dias_perm), 1) AS permanencia_media_dias
FROM processed
WHERE sistema = 'SIH'
