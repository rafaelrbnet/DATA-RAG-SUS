...
**Resultado:** `Linha 1: got={'cid_principal': 'S720', 'total_dias_internacao': 64885.0} exp={'cid_principal': 'S720', 'internacoes': 8980, 'total_dias_internacao': 64885}`
**TTA:** 5471ms
...
SELECT
  ROUND(
    ...
    1
  ) AS proporcao_internacoes_outro_municipio
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
```sql
SELECT
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / SUM(COALESCE(val_sp, 0)), 2) AS proporcão_val_sh_vs_val_sp
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'proporcão_val_sh_vs_val_sp': 472.05} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}`
**TTA:** 5830ms
...
WHERE sistema = 'SIH'
  AND ...`
- **Q26** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'S720', 'total_dias_internacao': 64885.0} exp={'cid_principal': 'S720', 'internacoes': 8980, 'total_dias_internacao': 64885}
...
FROM processed
WHERE sistema = 'SI...`
- **Q40** (Financeira): Linha 1: got={'proporcão_val_sh_vs_val_sp': 472.05} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}
  SQL: `SELECT
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / SUM(COALESCE(val_sp, 0)), 2) AS proporcão_val_sh_vs_val_sp
FROM proces...`
