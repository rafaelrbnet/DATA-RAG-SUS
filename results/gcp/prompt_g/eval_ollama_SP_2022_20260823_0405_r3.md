...
SELECT 
  cid_principal,
  COUNT(CASE WHEN obito_hospitalar = 'SIM' THEN 1 END) AS mortes,
  COUNT(*) AS total_internacoes,
  ROUND(100.0 * COUNT(CASE WHEN obito_hospitalar = 'SIM' THEN 1 END) / COUNT(*), 2) AS taxa_mortalidade
FROM 
  processed
WHERE 
  sistema = 'SIH'
...
LINE 3:   COUNT(CASE WHEN obito_hospitalar = 'SIM' THEN 1 END) AS mortes,
                          ^
...
**Resultado:** `Linha 1: got={'total_homens_idosos': 6060.0, 'total_mulheres_idosos': 13060.0, 'total_homens': 14000.0, 'total_mulheres': 15359.0} exp={'total_s72': 29356, 's72_idosos': 19120, 'pct_idosos': 65.1}`
**TTA:** 8555ms
...
**Resultado:** `Linha 1: got={'cid_principal': 'S063', 'permanencia_media_dias': 16.358974359} exp={'cid_principal': 'S063', 'internacoes': 78, 'permanencia_media': 16.4}`
**TTA:** 5309ms
...
**Resultado:** `Linha 1: got={'mes_cmpt': 8, 'total_obitos': 6006} exp={'mes_cmpt': 10, 'total_obitos': 224}`
**TTA:** 5770ms
...
SELECT
  mes_cmpt,
  ROUND(100.0 * SUM(CASE WHEN obito_internacao = 'S' THEN 1 ELSE 0 END) / COUNT(DISTINCT n_aih), 2) AS taxa_mortalidade
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Erro:** Falha DuckDB: Binder Error: Referenced column "obito_internacao" not found in FROM clause!
Candidate bindings: "tipo_financiamento", "dt_inter", "competencia_ano_mes", "mun_res_zona", "cid_princ"

LINE 3:   ROUND(100.0 * SUM(CASE WHEN obito_internacao = 'S' THEN 1 ELSE 0 END) / COUNT(DISTINCT...
                                      ^
...
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt ...`
- **Q09** (Epidemiológica Simples): got=40150 exp=40123
...
- **Q21** (Epidemiológica Complexa): Linha não encontrada: {'sexo_paciente': 'M', 'total': 40123, 'pct': 60.8}
  ...
  ROUND(100.0 * COUNT(*) / SUM(COUNT(*)) OVER (), 1) AS pct
FR...`
- **Q22** (Epidemiológica Complexa): Linha 1: got={'total_homens_idosos': 6060.0, 'total_mulheres_idosos': 13060.0, 'total_homens': 14000.0, 'total_mulheres': 15359.0} exp={'total_s72': 29356, 's72_idosos': 19120, 'pct_idosos': 65.1}
  SQL: `SELECT
  SUM(CASE WHEN sexo_paciente = 'M' AND idade_paciente >= 60 THEN 1 ELSE 0 END) AS total_homens_idosos,
  SUM(CAS...`
...
LINE 2:   ROUND(100.0 * COUNT(CASE WHEN cod_munic_paciente <> cod_munic_estabelecimento THEN 1 END...
                                        ^
  SQL: `SELECT 
  ROUND(100.0 * COUNT(CASE WHEN cod_munic_paciente <> cod_munic_estabelecimento THEN 1 END) / COUNT(*), 1) AS pc...`
...
LINE 3:   COUNT(CASE WHEN outcome = 'ÓBITO' THEN 1 END) AS obitos,
  ...
  sexo_paciente,
  COUNT(CASE WHEN outcome = 'ÓBITO' THEN 1 END) AS obitos,
  COUNT(DISTINCT n_aih) AS internaco...`
...
WHERE sistema = 'SIH'
 ...`
- **Q76** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'S063', 'permanencia_media_dias': 16.358974359} exp={'cid_principal': 'S063', 'internacoes': 78, 'permanencia_media': 16.4}
  SQL: `SELECT cid_principal,
  AVG(dias_perm) AS permanencia_media_dias
FROM processed
WHERE sistema = 'SIH'
...
- **Q99** (Financeira): Linha 1: got={'cid_principal': None, 'total_procedimentos': 181894, 'custo_total': 71949089.61} exp={'cod_procedimento': '0701020369', 'custo_total': 1201460.4}
  ...
  ROUND(SUM(COALESCE(custo_total, 0)), 2) AS custo_total
FROM p...`
- **Q101** (Temporal/Comparativa): Linha 1: got={'mes_cmpt': 8, 'total_obitos': 6006} exp={'mes_cmpt': 10, 'total_obitos': 224}
  SQL: `SELECT mes_cmpt,
  COUNT(DISTINCT n_aih) AS total_obitos
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  A...`
- **Q109** (Temporal/Comparativa): Falha DuckDB: Binder Error: Referenced column "obito_internacao" not found in FROM clause!
Candidate bindings: "tipo_financiamento", "dt_inter", "competencia_ano_mes", "mun_res_zona", "cid_princ"
...
LINE 3:   ROUND(100.0 * SUM(CASE WHEN obito_internacao = 'S' THEN 1 ELSE 0 END) / COUNT(DISTINCT...
  ...
  SQL: `SELECT
  mes_cmpt,
  ROUND(100.0 * SUM(CASE WHEN obito_internacao = 'S' THEN 1 ELSE 0 END) / COUNT(DISTINCT n_aih), 2) A...`
