...
| `sexo_paciente` | `pa_sexo` (SIA) ou `sexo` (SIH). |
| `raca_cor_paciente` | `pa_racacor` (SIA) ou `raca_cor` (SIH). |
| `cod_munic_residencia` | `pa_munpcn` (SIA) ou `munic_res` (SIH). |
| `cod_munic_estabelecimento` | `pa_ufmun` (SIA) ou `uf_zi` (SIH). |
| `cnes_estabelecimento` | `pa_coduni` (SIA) ou `cnes` (SIH). |
| `cod_procedimento` | `pa_proc_id` (SIA) ou `proc_rea` (SIH). |
| `cid_principal` | `pa_cidpri` (SIA) ou `diag_princ` (SIH). |
| `custo_total` | `pa_valapr` (SIA) ou `val_tot` (SIH). |
| `competencia_ano_mes` | `pa_cmp` (SIA) ou `ano_cmpt + mes_cmpt` (SIH), no formato `AAAAMM`. |
...
| 2 | `sexo_paciente` | Derivada (`pa_sexo` no SIA, `sexo` no SIH) | Sexo do paciente. |
| 3 | `raca_cor_paciente` | Derivada (`pa_racacor` no SIA, `raca_cor` no SIH) | Raça/cor autodeclarada do paciente. |
| 4 | `cod_munic_residencia` | Derivada (`pa_munpcn` no SIA, `munic_res` no SIH) | Código do município de residência do paciente. |
| 5 | `cod_munic_estabelecimento` | Derivada (`pa_ufmun` no SIA, `uf_zi` no SIH) | Código do município do estabelecimento/atendimento. |
| 6 | `cnes_estabelecimento` | Derivada (`pa_coduni` no SIA, `cnes` no SIH) | Identificador CNES do estabelecimento de saúde. |
| 7 | `cod_procedimento` | Derivada (`pa_proc_id` no SIA, `proc_rea` no SIH) | Código do procedimento realizado (SIGTAP). |
| 8 | `cid_principal` | Derivada (`pa_cidpri` no SIA, `diag_princ` no SIH) | Diagnóstico principal (CID-10) padronizado para análise. |
| 9 | `custo_total` | Derivada (`pa_valapr` no SIA, `val_tot` no SIH) | Valor financeiro principal para análise de custo. |
| 10 | `competencia_ano_mes` | Derivada (`pa_cmp` no SIA, `ano_cmpt + mes_cmpt` no SIH) | Competência no formato `AAAAMM`. |
...
| 15 | `main_icd` | Comum | CID principal padronizado. |
| 16 | `icd_group` | Comum | Grupo clínico derivado do CID. |
