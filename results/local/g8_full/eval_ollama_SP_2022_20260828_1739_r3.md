...
|---|---|
| Total queries | 118 |
| Correct (EA numerator) | 94 |
| **Execution Accuracy (EA)** | **79.7%** |
| Wilson IC 95% | [71.5%, 85.9%] |
| Correct via flexible match (alias diferente) | 76 |
| API errors / timeouts | 1 |
| Mean Time-to-Answer | 12.0s |
...
**Resultado:** `Linha 1: got={'cid_principal': 'S279', 'total_internacoes': 2, 'total_obitos': 2.0, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 29079ms
...
SELECT 
  ...
  (SELECT COUNT(DISTINCT n_aih) 
   ...
   WHERE sistema = 'SIH' 
     AND uf_origem = 'SP' 
     AND ano_cmpt = 2022 
     AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98')) AS total_internacoes_sp
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'S279', 'total_internacoes': 2, 'total_obitos': 2.0, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(DISTINCT n_aih) AS total_internacoes,
  SUM(morte) AS total_obitos,
