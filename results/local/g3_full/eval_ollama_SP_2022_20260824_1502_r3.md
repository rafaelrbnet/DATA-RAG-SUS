...
**Resultado:** `Linha 1: got={'cid_principal': 'T181', 'mortes': 1, 'internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 14633ms
...
**Resultado:** `Linha 1: got={'mes_cmpt': 1, 'total_ortopédico': 5898} exp={'mes_cmpt': 1, 'sistema': 'SIA', 'total': 5898}`
**TTA:** 8939ms
...
- **Q18** (Epidemiológica Complexa): Linha não encontrada: {'faixa_etaria': '0-17', 'total': 3619}
  ...
  CASE
    ...
    ELSE '60+'
  END AS...`
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T181', 'mortes': 1, 'internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(CASE WHEN morte = 1 THEN n_aih END) AS mortes,
  COUNT(DISTINCT n_aih) AS internacoes,
...
- **Q28** (Epidemiológica Complexa): Linha 1: got={'pct_internacoes_outro_municipio': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  ...
  ROUND(
    100.0 * COUNT(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN n_aih END) /
    COUN...`
- **Q29** (Epidemiológica Complexa): Linha 1: got={'mes_cmpt': 1, 'total_ortopédico': 5898} exp={'mes_cmpt': 1, 'sistema': 'SIA', 'total': 5898}
  SQL: `SELECT
  mes_cmpt,
  COUNT(*) AS total_ortopédico
FROM processed
