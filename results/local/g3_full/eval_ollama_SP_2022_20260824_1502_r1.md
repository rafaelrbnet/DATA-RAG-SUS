...
SELECT 
  ...
  COUNT(DISTINCT n_aih) AS total_internacoes,
  ROUND(100.0 * COUNT(CASE WHEN morte = 1 THEN 1 END) / COUNT(DISTINCT n_aih), 1) AS taxa_mortalidade
FROM 
  processed
WHERE 
  sistema = 'SIH'
...
**Resultado:** `Linha 1: got={'cid_principal': 'S45', 'mortes': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 14211ms
...
```sql
SELECT 
  COUNT(DISTINCT n_aih) AS internacoes_idosos,
  (SELECT COUNT(DISTINCT n_aih) 
   FROM processed 
   WHERE sistema = 'SIH' 
     AND uf_origem = 'SP' 
     AND ano_cmpt = 2022 
     AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98')) AS total_internacoes
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'internacoes_idosos': 19117, 'total_internacoes': 65970} exp={'total_s72': 29356, 's72_idosos': 19120, 'pct_idosos': 65.1}`
**TTA:** 15617ms
...
SELECT 
  ROUND(100.0 * 
    ...
    COUNT(DISTINCT n_aih), 1) AS pct_internacoes_outro_municipio
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
SELECT
  mes_cmpt,
  COUNT(DISTINCT CASE WHEN sistema = 'SIA' THEN n_aih END) AS total_sia,
  COUNT(DISTINCT CASE WHEN sistema = 'SIH' THEN n_aih END) AS total_sih
FROM processed
WHERE uf_origem = 'SP'
  AND ano_cmpt = 2022
  AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98')
...
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'S45', 'mortes': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(CASE WHEN morte = 1 THEN 1 END) AS mortes,
  COUNT(DISTINCT n_aih) AS total_internacoes...`
- **Q22** (Epidemiológica Complexa): Linha 1: got={'internacoes_idosos': 19117, 'total_internacoes': 65970} exp={'total_s72': 29356, 's72_idosos': 19120, 'pct_idosos': 65.1}
  SQL: `SELECT 
  COUNT(DISTINCT n_aih) AS internacoes_idosos,
  (SELECT COUNT(DISTINCT n_aih) 
   FROM processed 
