...
SELECT 
    ...
    COUNT(DISTINCT n_aih) AS total_internacoes_sp
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'internacoes_idosos': 19117, 'total_internacoes_sp': 29356} exp={'total_s72': 29356, 's72_idosos': 19120.0, 'pct_idosos': 65.1}`
**TTA:** 10616ms
...
```sql
SELECT 
  ROUND(100.0 * SUM(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN 1 ELSE 0 END) / COUNT(DISTINCT n_aih), 1) AS proporcao_internacoes_externas
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
SELECT 
  ...
  COUNT(CASE WHEN icd_group = 'M00-M99' THEN n_aih END) AS internacoes_musculoesqueletica
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `got=2077477 exp=173.1`
**TTA:** 11456ms
...
- **Q18** (Epidemiológica Complexa): Linha não encontrada: {'faixa_etaria': '0-17', 'total': 3619}
  ...
  CASE 
    WHEN idade_paciente < 18 THEN '0-17'
    WHEN idade_paciente >= 18 AND idade_paciente <= 59 THEN '18...`
...
- **Q22** (Epidemiológica Complexa): Linha 1: got={'internacoes_idosos': 19117, 'total_internacoes_sp': 29356} exp={'total_s72': 29356, 's72_idosos': 19120.0, 'pct_idosos': 65.1}
  SQL: `SELECT 
    COUNT(DISTINCT CASE WHEN idade_paciente >= 60 THEN n_aih END) AS internacoes_idosos,
    COUNT(DISTINCT n_ai...`
...
- **Q28** (Epidemiológica Complexa): Linha 1: got={'proporcao_internacoes_externas': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219.0, 'pct_deslocamento': 56.4}
  SQL: `SELECT 
  ROUND(100.0 * SUM(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN 1 ELSE 0 END) / COUNT(DISTI...`
...
- **Q79** (Epidemiológica Complexa): Número de linhas: got=5 exp=3
  ...
  CASE
    ...
    WHEN idade_paciente BETWEEN 18 AND 34 THEN '18-34'
    WHEN i...`
- **Q83** (Epidemiológica Complexa): got=2077477 exp=173.1
  SQL: `SELECT 
  cnes_estabelecimento,
  COUNT(DISTINCT n_aih) AS internacoes
FROM 
