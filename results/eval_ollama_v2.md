...
| Correct via flexible match (alias diferente) | 26 |
| API errors / timeouts | 0 |
| Mean Time-to-Answer | 32.7s |
...
**Resultado:** `Linha 1: got={'cid_principal': 'T509', 'total_obitos': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 36613ms
...
**Resultado:** `Linha 1: got={'proporcao_internacoes_outro_municipio': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}`
**TTA:** 31707ms
...
SELECT 
  mes_cmpt,
  COUNT(*) AS total_sia,
  (SELECT COUNT(DISTINCT n_aih) FROM processed WHERE sistema = 'SIH' AND uf_origem = 'SP' AND ano_cmpt = 2022 AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98') AND mes_cmpt = p.mes_cmpt) AS total_sih
FROM processed p
WHERE sistema = 'SIA'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T509', 'total_obitos': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(CASE WHEN morte = 1 THEN n_aih END) AS total_obitos,
...
WHERE sistema = 'SIH'
  AND uf_orige...`
- **Q28** (Epidemiológica Complexa): Linha 1: got={'proporcao_internacoes_outro_municipio': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  SQL: `SELECT 
