...
```sql
SELECT
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / SUM(COALESCE(val_sp, 0)), 2) AS proporc_custo_sh_vs_sp
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'proporc_custo_sh_vs_sp': 472.05} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}`
**TTA:** 5863ms
...
SELECT
  ...
  COUNT(DISTINCT n_aih) AS total_internacoes,
  SUM(morte) AS total_obitos,
  ROUND(100.0 * SUM(morte) / COUNT(DISTINCT n_aih), 1) AS taxa_obitos
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
FROM processed
WHERE sistema = 'SI...`
- **Q40** (Financeira): Linha 1: got={'proporc_custo_sh_vs_sp': 472.05} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}
  SQL: `SELECT
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / SUM(COALESCE(val_sp, 0)), 2) AS proporc_custo_sh_vs_sp
FROM processed
...`
