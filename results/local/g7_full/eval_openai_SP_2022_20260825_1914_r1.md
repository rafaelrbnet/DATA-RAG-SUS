...
**Resultado:** `Linha 1: got={'cid_principal': 'T008', 'total_internacoes': 1, 'total_obitos': 1.0, 'taxa_mortalidade_pct': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 6596ms
...
```sql
SELECT
  ROUND(100.0 * COUNT(DISTINCT p.n_aih) / SUM(COUNT(DISTINCT p.n_aih)) OVER (), 1) AS pct_deslocamento_intermunicipal
FROM processed p
...
**Resultado:** `Linha 1: got={'pct_deslocamento_intermunicipal': 100.0} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}`
**TTA:** 10069ms
...
```sql
SELECT faixa_etaria,
  ROUND(SUM(custo_total) / COUNT(DISTINCT n_aih), 2) AS custo_medio
FROM (
  SELECT n_aih,
    custo_total,
    CASE
...
- **Q18** (Epidemiológica Complexa): Linha não encontrada: {'faixa_etaria': '0-17', 'total': 3619}
  ...
  CASE
    ...
    WHEN idade_paciente BETWEEN 18 AND 59 THEN '18-59'
    ELSE '...`
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T008', 'total_internacoes': 1, 'total_obitos': 1.0, 'taxa_mortalidade_pct': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
...
WHERE sistema = 'SIH'
  AND uf_origem = '...`
- **Q28** (Epidemiológica Complexa): Linha 1: got={'pct_deslocamento_intermunicipal': 100.0} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  SQL: `SELECT
  ROUND(100.0 * COUNT(DISTINCT p.n_aih) / SUM(COUNT(DISTINCT p.n_aih)) OVER (), 1) AS pct_deslocamento_intermunic...`
