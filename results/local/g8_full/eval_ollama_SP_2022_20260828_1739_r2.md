...
| Correct via flexible match (alias diferente) | 75 |
| API errors / timeouts | 0 |
| Mean Time-to-Answer | 12.5s |
...
**Resultado:** `Linha 1: got={'cid_principal': 'T181', 'total_internacoes': 1, 'total_obitos': 1.0, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 29123ms
...
SELECT 
  ROUND(
    100.0 * 
    SUM(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN 1 ELSE 0 END) / 
...
**Resultado:** `Linha 1: got={'pct_internacoes_com_deslocamento': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}`
**TTA:** 12498ms
...
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T181', 'total_internacoes': 1, 'total_obitos': 1.0, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(DISTINCT n_aih) AS total_internacoes,
  SUM(morte) AS total_obitos,
...
WHERE sistema = 'SIH'
  AND uf_origem = '...`
- **Q28** (Epidemiológica Complexa): Linha 1: got={'pct_internacoes_com_deslocamento': 56.4} exp={'total_internacoes': 65970, 'deslocamento_intermunicipal': 37219, 'pct_deslocamento': 56.4}
  SQL: `SELECT 
  ROUND(
    100.0 * 
    SUM(CASE WHEN cod_munic_residencia != cod_munic_estabelecimento THEN 1 ELSE 0 END) / 
