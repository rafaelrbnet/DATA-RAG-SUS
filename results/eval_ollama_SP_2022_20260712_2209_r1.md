...
| Correct via flexible match (alias diferente) | 60 |
...
**SQL gerado:**
```sql
SELECT COUNT(DISTINCT n_aih) AS total_internacoes_femininas
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'cid_principal': 'T181', 'total_obitos': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21.0, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 15081ms
...
SELECT 
  mes_cmpt,
  COUNT(*) AS total_ortopedico_sia,
  (SELECT COUNT(DISTINCT n_aih) FROM processed WHERE sistema = 'SIH' AND uf_origem = 'SP' AND ano_cmpt = 2022 AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98') AND mes_cmpt = p.mes_cmpt) AS total_ortopedico_sih
FROM processed p
WHERE sistema = 'SIA'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'cid_principal': 'S221', 'custo_medio': 14053.8802439024, 'total_internacoes': 41} exp={'cid_principal': 'S221', 'internacoes': 41, 'custo_medio': 14053.88}`
**TTA:** 10836ms
...
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T181', 'total_obitos': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21.0, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(CASE WHEN morte = 1 THEN n_aih END) AS total_obitos,
...
- **Q93** (Financeira): Número de linhas: got=5 exp=3
  ...
  CASE 
    ...
    WHEN idade_paciente BETWEEN 18 AND 34 THEN '18-34'
    WHEN...`
- **Q97** (Financeira): Linha 1: got={'cid_principal': 'S221', 'custo_medio': 14053.8802439024, 'total_internacoes': 41} exp={'cid_principal': 'S221', 'internacoes': 41, 'custo_medio': 14053.88}
  SQL: `SELECT cid_principal,
       AVG(custo_total) AS custo_medio,
       COUNT(DISTINCT n_aih) AS total_internacoes
FROM pro...`
