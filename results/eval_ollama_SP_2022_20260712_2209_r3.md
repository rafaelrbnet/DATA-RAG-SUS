...
**Resultado:** `Linha 1: got={'cid_principal': 'T509', 'total_obitos': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21.0, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 14469ms
...
SELECT 
  mes_cmpt,
  COUNT(*) AS total_sia,
  SUM(CASE WHEN sistema = 'SIA' THEN 1 ELSE 0 END) AS sia_internacoes,
  SUM(CASE WHEN sistema = 'SIH' THEN 1 ELSE 0 END) AS sih_internacoes
FROM processed
WHERE uf_origem = 'SP'
  AND ano_cmpt = 2022
  AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98')
...
```sql
SELECT 
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / (SUM(COALESCE(val_sh, 0)) + SUM(COALESCE(val_sp, 0))), 2) AS pct_val_sh,
  ROUND(100.0 * SUM(COALESCE(val_sp, 0)) / (SUM(COALESCE(val_sh, 0)) + SUM(COALESCE(val_sp, 0))), 2) AS pct_val_sp
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
WHERE sistema = 'SIH'
  ...
  AND cid_principal LIKE 'S72%'
GROUP BY cnes_estabelecimento
ORDER BY internacoes_femur DESC
LIMIT 5;
```
...
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T509', 'total_obitos': 1, 'total_internacoes': 1, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21.0, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(CASE WHEN morte = 1 THEN n_aih END) AS total_obitos,
...
- **Q40** (Financeira): Linha 1: got={'pct_val_sh': 82.52, 'pct_val_sp': 17.48} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}
  SQL: `SELECT 
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / (SUM(COALESCE(val_sh, 0)) + SUM(COALESCE(val_sp, 0))), 2) AS pct_val_...`
