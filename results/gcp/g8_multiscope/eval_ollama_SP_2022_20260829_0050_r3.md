...
**Resultado:** `Linha 1: got={'cid_principal': 'T008', 'total_internacoes': 1, 'total_obitos': 1.0, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}`
**TTA:** 10708ms
...
SELECT 
  ...
  (SELECT COUNT(DISTINCT n_aih) FROM processed WHERE sistema = 'SIH' AND uf_origem = 'SP' AND ano_cmpt = 2022 AND (icd_group = 'M00-M99' OR icd_group = 'S00-T98')) AS total_internacoes_sp
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**SQL gerado:**
```sql
SELECT ROUND(SUM(COALESCE(custo_total, 0)) / SUM(COALESCE(qt_diarias, 1)), 2) AS custo_medio_por_dia
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
```sql
SELECT 
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / SUM(COALESCE(val_sp, 0)), 2) AS proporcao_custo_sh_vs_honorarios
FROM processed
WHERE sistema = 'SIH'
  AND uf_origem = 'SP'
  AND ano_cmpt = 2022
...
**Resultado:** `Linha 1: got={'proporcao_custo_sh_vs_honorarios': 472.05} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}`
**TTA:** 5923ms
...
**Resultado:** `Linha não encontrada: {'sexo_paciente': 'M', 'total_obitos': 1142.0, 'taxa_mortalidade': 2.85}`
**TTA:** 7448ms
...
- **Q19** (Epidemiológica Complexa): Linha 1: got={'cid_principal': 'T008', 'total_internacoes': 1, 'total_obitos': 1.0, 'taxa_mortalidade': 100.0} exp={'cid_principal': 'S062', 'total_internacoes': 70, 'total_obitos': 21, 'taxa_mortalidade_pct': 30.0}
  SQL: `SELECT 
  cid_principal,
  COUNT(DISTINCT n_aih) AS total_internacoes,
  SUM(morte) AS total_obitos,
...
- **Q37** (Financeira): got=488.68 exp=441.95
  SQL: `SELECT ROUND(SUM(COALESCE(custo_total, 0)) / SUM(COALESCE(qt_diarias, 1)), 2) AS custo_medio_por_dia
FROM processed
WHER...`
...
FROM processed
WHERE sistema = 'SI...`
- **Q40** (Financeira): Linha 1: got={'proporcao_custo_sh_vs_honorarios': 472.05} exp={'custo_servico_hospitalar': 144636796.74, 'honorarios_profissionais': 30640001.52, 'pct_hospitalar': 82.5}
  SQL: `SELECT 
  ROUND(100.0 * SUM(COALESCE(val_sh, 0)) / SUM(COALESCE(val_sp, 0)), 2) AS proporcao_custo_sh_vs_honorarios
FROM...`
...
- **Q74** (Epidemiológica Complexa): Linha não encontrada: {'sexo_paciente': 'M', 'total_obitos': 1142.0, 'taxa_mortalidade': 2.85}
  SQL: `SELECT 
  sexo_paciente,
  COUNT(DISTINCT n_aih) AS total_internacoes,
  SUM(morte) AS total_obitos,
